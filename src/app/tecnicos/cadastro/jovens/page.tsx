...
import { GraduationCap, ArrowRight, ArrowLeft, RefreshCw, CheckCircle } from 'lucide-react';
import { db, PIRAPORA_BAIRROS } from '../../../../lib/db';
import type { ReferenceUnit } from '../../../../lib/db';

export default function TecnicosCadastroJovensPage() {
  ...
  // 1. Estado inicial totalmente mapeado e garantido
  ...
  const handleNextStep = () => {
    if (regStep === 1) {
      if (!regData.nome_completo.trim()) {
        showToast('⚠️ Por favor, preencha o Nome Completo.');
        return;
      }
      if (!regData.cpf.trim()) {
        showToast('⚠️ Por favor, preencha o CPF.');
        return;
      }
    }
  ...
  const handleRegisterYouth = (e: React.FormEvent) => {
    ...
    // Se não estiver na última etapa (Etapa 4), não permite submeter o formulário
    if (regStep < 4) {
      handleNextStep();
      return;
    }

    // Validação da Etapa 1 (caso o usuário volte e apague)
    ...
    // Validação da Etapa 4 (Interesses)
    if (!regData.area_interesse) {
      showToast('⚠️ Por favor, selecione uma área de interesse na Etapa 4.');
      return;
    }

    if (regData.area_interesse === 'Outros' && !regData.outra_area_interesse.trim()) {
      showToast('⚠️ Por favor, especifique a área de interesse no campo de texto.');
      return;
    }
  ...
  return (
    <div className="flex flex-col gap-6 animate-fadeIn relative">
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col gap-2">
        <h2 className="text-xl font-black text-slate-900">Cadastrar Jovem no Programa</h2>
        ...
        <form onSubmit={handleRegisterYouth} className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4 animate-fadeIn">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1 md:col-span-2">
                  <label className="text-[10px] font-extrabold text-slate-500 uppercase">Endereço (Rua)</label>
                  <input 
                    type="text" 
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-[10px] font-extrabold text-slate-500 uppercase">Número da Casa</label>
                  <input 
                    type="text" 
                    value={regData.numero_casa || ''}
                    onChange={(e) => setRegData(prev => ({ ...prev, numero_casa: e.target.value }))}
          ...
          {regStep === 4 && (
            <div className="flex flex-col gap-4 animate-fadeIn">
              ...
              {/* Seção Corrigida com Validação Segura */}
              <div className="flex flex-col gap-2 mt-2">
                <label className="text-xs font-extrabold text-slate-700">Qual a área de maior interesse profissional? *</label>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {['Administrativo', 'Comércio', 'Tecnologia', 'Logística', 'Indústria', 'Saúde', 'Outros'].map(area => {
                    const isSelected = (regData?.area_interesse || '') === area;
                    return (
                      <div 
                        key={area}
                        onClick={() => setRegData(prev => ({ ...prev, area_interesse: area }))}
                        className={`p-3 rounded-xl border flex items-center gap-2.5 cursor-pointer transition-colors font-bold text-xs ${
                          isSelected 
                            ? 'bg-indigo-50 border-indigo-500 text-indigo-700' 
                            : 'bg-slate-50 border-slate-100 text-slate-600 hover:bg-slate-100'
                        }`}
                      >
                        {/* Correção de tamanho padrão (h-4 w-4) para evitar colapso visual */}
                ...
                {/* Input Condicional Seguro */}
                {(regData?.area_interesse || '') === 'Outros' && (
                  <div className="mt-3 flex flex-col gap-1 animate-fadeIn">
                    <label className="text-[10px] font-extrabold text-slate-500 uppercase">Especifique a área de interesse *</label>
                    <input 
                      type="text" 
                      value={regData.outra_area_interesse || ''}
                      onChange={(e) => setRegData(prev => ({ ...prev, outra_area_interesse: e.target.value }))}
                      placeholder="Ex: Artes, Música, Esportes..."
                      className="border border-slate-200 rounded-xl px-4 py-3 text-sm font-semibold text-slate-800 focus:outline-indigo-500 bg-white shadow-inner"
                    />
                  </div>
                )}
              </div>
